...
outgoing.setup = function (host, port, user, pass, secure, overideFromAddress) {
  // defaults
  ...
  config.overideFromAddress = overideFromAddress
...
function relayMail (emailObject, emailStream, isAutoRelay, done) {
  ...
  let sender = getAddressFromAddressObject(emailObject.envelope.from)
  if (typeof config.overideFromAddress === 'string') {
    sender = config.overideFromAddress
  }
